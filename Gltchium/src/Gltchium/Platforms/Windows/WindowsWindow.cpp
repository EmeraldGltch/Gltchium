...
namespace Gltchium {
	...
	static void GLFWErrorCallback(int error, const char* description) {
		GC_CORE_ERROR("GLFW Error ({0}): {1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props) {
		...
		GC_CORE_INFO("Creating Window {0} ({1}x{2})", props.Title, props.Width, props.Height);

		if (!s_GLFWInitialized) {
			int success = glfwInit();
			GC_CORE_ASSERT(success, "Could not intialize GLFW!");
